/*
This file will help you program easily that it contains some functions which are used often.
===============================================
NEW FUNCTIONS WIP!!!
If you find bugs in these completed functions,please contact me!
My Website:https://github.com/Entity303-E3/C-mods
===============================================
Version:1.3.3
Updating log:
2021/07/22: Update to 1.0.0
	create this mod
2021/07/22: Update to 1.0.1
	fix some bugs(change_x_2_dec(),change_x_2_y())
2021/07/27: Update to 1.0.2
	improve the method of calculation(sum())
2021/07/27: Update to 1.1.0
	add personal program materials(ALL FREE!)
2021/07/29: Update to 1.2.0
	add 2 functions(is_prime(),show_factor())
2021/07/31: Update to 1.2.1
	improve the calculation method of some functions(is_prime	(),show_factor())
2021/08/05: Update to 1.3.0
	add 2 functions(gcd(),lcm())
2021/08/05: Update to 1.3.1
	improve the program of a function(is_prime())
2021/08/07: Update to 1.3.2
	improve the calculating method of a function(is_prime())
2021/08/07: Update to 1.3.3
	fix a bug of a function(change_x_2_dec())
===============================================
CONTENTS:
----------------------------------Hexadecimal--------------------------------
	change_2_bin(int)	return string:
		change decimal into binary

	change_2_dec(string)	      return int:
		change binary into decimal

	change_x_2_dec(string,int)	return int:
		change a number into decimal

	change_x_2_y(string,int,int)	return string:
		change a number into another number
ATTENTION:ALL FUNCTIONS UNSIGNED!
--------------------------------------------------------------------------------

---------------------------------------Data------------------------------------
	change_str_2_int(string)	return long long:(unsigned)
		change a string into long long(19 BITS MAX!!!)
--------------------------------------------------------------------------------

-----------------------------------Judging-------------------------------------
	str_in_str(string,string)	return bool:
		judge whether a string is in another string
	is_prime(long long)	return bool:
		judge whether an integal is a prime
--------------------------------------------------------------------------------

----------------------------------Calculation----------------------------------
	sum(long long,  long long，long long）	return long long:
		sum from a to b, once adding c
	show_factor(int)	return none:
		output all prime factors of an integal
	gcd(long long,long long)	return long long:
		calculate the greatest common division of 2 		numbers
	lcm(long long,long long)	return long long:
		calculate the least common multiple of 2 numbers
--------------------------------------------------------------------------------
===============================================
=================Programs Here!!!!!!!!!!===============*/
#include<bits/stdc++.h>
using namespace std;
string change_2_bin(int a){
	string b,d;
	int c=0;
	while(a){
		b+=a%2+48;
		a/=2;
		c++;
	}
	c=0;
	for(int i=b.length()-1;i>=0;i--){
		d+=b[i];
		c++;
	}
	return d;
}
int change_2_dec(string a){
	int b,num=0,cnt=1;
	for(int i=a.length()-1;i>=0;i--){
		b=a[i]-48;
		num+=cnt*b;
		cnt*=2;
	}
	return num;
}
int change_x_2_dec(string a,int s){
	if(s<=36){
		int b,t,num=0,cnt=1;
		for(int i=a.length()-1;i>=0;i--){
			t=a[i];
			if(t<58){
				b=t-48;
			}
			else if(t>=58 and t<=90){
				b=t-55;
			}
			else{
				b=t-87;
			}
			num+=cnt*b;
			cnt*=s;
		}
		return num;
	}
	else{
		return -1;
	}
}
string change_x_2_y(string a,int b,int c){
	int dec_num=change_x_2_dec(a,b);
	if(c<=36){
		string x,y;
		int s=0;
		while(dec_num){
			if(dec_num%c>=10){
				x+=dec_num%c+55;
				dec_num/=c;
				s++;
			}
			else{
				x+=dec_num%c+48;
				dec_num/=c;
				s++;
			}
		}
		s=0;
		for(int i=x.length()-1;i>=0;i--){
			y+=x[i];
			s++;
		}
	return y;
	}
	else{
		return "DATA OUT OF RANGE!";
	}
}
long long change_str_2_int(string a){
	long long num=0,t,cnt=1;
	for(int i=a.length()-1;i>=0;i--){
		t=a[i]-48;
		num+=t*cnt;
		cnt*=10;
	}
	return num;
}
bool str_in_str(string a,string b){
	if(a.length()>b.length()){
		return false;
	}
	else{
		string index="";
		for(int i=0;i<=b.length()-a.length();i++){
			index="";
			for(int j=i;j<i+a.length();j++){
				index+=b[j];
			}
			if(a==index){
			return true;
			}
		}
		return false;
	}
}
bool is_prime(long long a){
	if(a==1 or a==0){
		return false;
	}
	for(long long i=2;i*i<=a;i++){
		if(a%i==0){
			return false;
		}
	}
	return true;
}
long long sum(long long a,long long b,long long c){
	long long num=(a+b)*((b-a)/c+1)/2;
	return num;
}
void show_factor(long long a){
	long long i=2;
	while(i*i<=a){
		while(a%i==0){
			cout<<i<<" ";
			a/=i;
		}
		i++;
	}
	if(a>1){
		cout<<a;
	}
	return;
}
long long gcd(long long a,long long b){
	if(a>b){
		for(long long i=a/2;i>=1;i--){
			if(a%i==0 and b%i==0){
				return i;
			}
		}
	}
	else{
		for(long long i=b/2;i>=1;i--){
			if(a%i==0 and b%i==0){
				return i;
			}
		}
	}
	return -1;
}
long long lcm(long long a,long long b){
	return a*b/gcd(a,b);
}
/*=================Program Materials=================
--------------------------------------------------------------------------------
Choose_Sort:
#include<bits/stdc++.h>
using namespace std;
int main(){
	int n,mn,id,t;
	cin>>n;
	int a[n];
	for(int i=0;i<n;i++){
		cin>>a[i];
	}
	for(int i=0;i<n-1;i++){
		mn=a[i];
		id=i;
		for(int j=i+1;j<n;j++){
			if(mn>a[j]){
				mn=a[j];
				id=j;
			}
		}
		t=a[id];
		a[id]=a[i];
		a[i]=t;
	}
	for(int i=0;i<n;i++){
		cout<<a[i]<<" ";
	}
}
--------------------------------------------------------------------------------
Bubble_Sort:
#include<bits/stdc++.h>
using namespace std;
int main(){
	int n;
	bool flag=1;
	cin>>n;
	int a[n];
	for(int i=0;i<n;i++){
		cin>>a[i];
	}
	for(int i=0;i<n-1;i++){
		flag=0;
		for(int j=0;j<n-1-i;j++){
			if(a[j]>a[j+1]){
				swap(a[j],a[j+1]);
				flag=1;
			}
		}
		if(not flag){
			break;
		}
	}
	for(int i=0;i<n;i++){
		cout<<a[i]<<" ";
	}
}
--------------------------------------------------------------------------------
Insert_Sort:
#include<bits/stdc++.h>
using namespace std;
int main(){
	int n;
	cin>>n;
	int a[n];
	for(int i=0;i<n;i++){
		cin>>a[i];
	}
	for(int i=1;i<n;i++){
		for(int j=i;j>=1;j--){
			if(a[j]<a[j-1]){
				swap(a[j],a[j-1]);
			}
		}
	}
	for(int i=0;i<n;i++){
		cout<<a[i]<<" ";
	}
}
--------------------------------------------------------------------------------
=============================================*/